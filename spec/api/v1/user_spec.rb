  ...
  describe 'POST /api/v1/users' do
   ...
   describe 'POST /api/v1/users/change_password' do
    context 'Valid params' do
      it 'Valid params' do
        post '/api/v1/users/sign_in', params: { email: @user.email, password: '123456' }
        post '/api/v1/users/change_password', params: { current_password: '123456', new_password: '123789', confirm_password: '123789' }, headers: { Authorization: JSON.parse(response.body)['access_token'] }
        expect(response).to be_success
    ...
    context 'Invalid params' do
      it 'current_password' do
        post '/api/v1/users/sign_in', params: { email: @user.email, password: '123456' }
        post '/api/v1/users/change_password', params: { current_password: 'KKKHHH', new_password: '123456123456', confirm_password: '123456123456' }, headers: { Authorization: JSON.parse(response.body)['access_token'] }
        expect(response).to be_success
      ...
      it 'new_password' do
        post '/api/v1/users/sign_in', params: { email: @user.email, password: '123456' }
        post '/api/v1/users/change_password', params: { current_password: '123456', new_password: 'KKDDSS', confirm_password: '123456123456' }, headers: { Authorization: JSON.parse(response.body)['access_token'] }
        expect(response).to be_success
        expect(JSON.parse(response.body)['success']).to eq(false)
        expect(JSON.parse(response.body)['message']).to eq('Password has not been changed')
      end
